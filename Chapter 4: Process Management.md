...
The structure of `TSS` descriptor in `GDT` table is same as the segment descriptor that we have already explained in chapter \ref{ch-x86}. The only difference is in the *type field* which has the static binary value `010B1` in `TSS` descriptor where `B` in this value is known as `B` flag, or *busy flag* which should be `1` when the process that this TSS descriptor represents is active and `0` when it is inactive.
...
[^5]: In chapter \ref{ch-x86} we have seen that there are two types of segments in x86, application segments such as code, data and stack segment. And system segments and they are `LDT` and `TSS`.
