...
Figure <!-- TODO: figure --> shows the structure of a task-state segment, as you can see, most of the fields are values of registers while the others are out of our topic's range except for previous task link which will be covered in a moment. You can see that stack segment register and stack pointer register have four entries instead of one, `SS`, `SS0`, `SS1` and `SS2` for stack segment register. `ESP`, `ESP0`, `ESP1` and `ESP2` for stack pointer register. These fields points to the stack that should be used when the process is in a specific privilege level, for example, `SS0:ESP0` will be used as the stack of the process when it switches to privilege level `0`, when it switches back to privilege level `3` the stack `SS:ESP` will be used instead, and the same is applicable to the other similar fields. If we intend to implement software multitasking, the sole reason of defining at least one TSS is due to these fields, when a switch between privilege levels occurs, the processor needs a TSS to use these fields from it in order to switch between stacks. This is needed only when the system runs user-space code, that is, privilege level `3` code.

Figure <!-- TODO: figure --> shows the structure of TSS descriptor in GDT table. As you can see, all fields and flags are same as the ones that we have already explained in chapter <!-- [REF] 3 -->. The differences are in the  *type field* which has static values in TSS descriptor and a flag called `B`, or *busy flag* which should be `1` when the process that this TSS descriptor represents is active and `0` when it is inactive.
